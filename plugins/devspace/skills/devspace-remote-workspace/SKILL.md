...
- Representative repositories: `opencodex`, `aura`, and `aura-board`. Other repositories may be opened when they are inside the configured allowed root.
- GitHub authentication: repository-specific SSH deploy keys owned by `devspace`

Call `open_workspace` once for the requested repository and reuse its `workspaceId`. Prefer the Codex-compatible `read`, `write`, `edit`, `apply_patch`, `exec_command`, `write_stdin`, and `git` tools when they are exposed. Older installations may expose `grep`, `glob`, `ls`, and `bash` instead; use those names only when they are actually present. `devspace_projects` lists the root and repository paths. Paths passed to workspace tools are relative to the opened repository and cannot escape it.

## Git workflow

1. Call `open_workspace` for the requested repository under the configured project root. `opencodex`, `aura`, and `aura-board` are representative aliases, not an exhaustive repository allowlist.
2. Inspect the relevant files and current branch/worktree state.
3. Make focused edits through `edit` or `write`.
4. Run targeted tests or checks through `exec_command`; use `write_stdin` to poll long-running commands. On older installations, use `bash` with an appropriate timeout.
